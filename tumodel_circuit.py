...
unitary1 = get_faulty_CZ('delay', 0.1,False)
fid = qutip.average_gate_fidelity(
            qutip.Qobj(unitary1, dims=[[2, 2], [2, 2]]),
...
def hyp_testing(num_qubits, qc, fault_info, ini_state):
    ...
    U_list = faulty_propagator(num_qubits, qc, fault_info, expand = True)
    ...
    for k in range(N):
        ...
        while(quit_sig == 0 and count < 5e3): 
            ...
            if(np.isin(sample, valid_index) == False):
                ...
                if(chi2_real > chi2.ppf(0.99, length - 1)):
                    quit_sig = 1
        #print(count)
        count_avg += count
...
test_rep_duration_rand = np.zeros(2**num_qubits, dtype = int)
test_rep_sz_rand = np.zeros(2**num_qubits, dtype = int)
test_rep_coup_rand = np.zeros(2**num_qubits, dtype = int)
test_rep_delay_rand = np.zeros(2**num_qubits, dtype = int)
for index, state in enumerate(states):
    test_rep_duration_rand[index] = math.ceil(hyp_testing(num_qubits, qc, ['duration',0.1], state))
    test_rep_sz_rand[index] = math.ceil(hyp_testing(num_qubits, qc, ['amp_sz',0.1], state))
    test_rep_coup_rand[index] = math.ceil(hyp_testing(num_qubits, qc, ['amp_coup',0.1], state))
    test_rep_delay_rand[index] = math.ceil(hyp_testing(num_qubits, qc, ['delay',0.1], state))
#%%
print(test_rep_delay_rand)
print(test_rep_sz_rand)
print(test_rep_coup_rand)
print(test_rep_delay_rand)
# %%
...
for i in range(16):
    plt.bar(X[i], test_rep_duration_rand[i],color = 'cornflowerblue', width = 0.6)
    
for x,y in zip(X,test_rep_duration_rand):
...
plt.savefig('pattern-repduration10-rand.eps')
# %%
