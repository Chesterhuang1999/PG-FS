# %% Basic Imports and definitions for superconducting quantum model used in the experiment
...
def random_circuit(N = 4):
    """
    a randomly-sampled quantum supremacy circuit. Single qubit gates are 
    sampled from the set { \sqrt{X}, \sqrt{Y}, \sqrt{(X+Y)/\sqrt{2}}} }
...
def get_faulty_CZ(fault_type, fault_arg, ZI_pulse = False):
    ...
    U = U_list[-1]
    # Special handling for delay fault
    if(fault_type == 'delay'):
...
### We try to introduce a small trick to help get the state
### that best distinguishes the faulty and fault-free CZ gate. 
### Not limited to CZ; other gates could also be applied.
...
def get_optim_state(fault_info):
    ...
    return optim_state
#print(get_optim_state(["amp_coup", 0.1]))
#%%
...
def faulty_propagator(num_qubits, qc, fault_info, expand = True):
    ...
    Out:
    U_list: gate lists for the faulty circuit.
    fault_index: the location of the faulty gate
    targets : targets of the fautly CZ gate, used in back propagation.
    ...
    for index, gate in enumerate(gates):
        ...
        if gate.name in qc.user_gates:
            ...
            if expand:
                ...
                elif gate.name == 'CSIGN' and cz_count != 0:
                    cz_count = cz_count + 1
...
#%%
### Back propagation process for pre select the test patterns. 
### Only applicable to faults in the first half. 
### For the second half, reverse the circuit and the fault can also be detected.
def back_propagation(num_qubits, qc, fault_info):
    """
    Reproduce the optimal test pattern for the circuit, provided with the fault information (location and scale) 
    """
    N = num_qubits
    ...
    state_amp = np.zeros(2**N)
    for j in range(2**N):
        state_amp[j] = abs(state_exp[j])**2
    return state_amp
...
def traverse_states(num_qubits, res_info = [0, 0]):
    ...
    res_info: form of [x, y]. setting the value of position x to be y,
    then generate all the states that satisfy the requirement.
    """
    n = num_qubits
    N = 2**n
...
#%%
#We vaildate the pre-selection protocol by randomly generate supremacy circuits and quantum Draper Adder in 4-qubit and 6-qubit cases.
def protocol_val(num_qubits):
    N = num_qubits
    qc_add = draper_adder(N)
    qc_add = qc_add.resolve_gates(basis = ["RX","RY","CSIGN"])
    states = traverse_states(N, [0, 0])
  
    is_match_delay = 0
    is_match_dur = 0
    #Probabilities created by pre-selection
    pre_sele_dur = back_propagation(N, qc_add, ['duration', 0.1])
    pre_sele_delay = back_propagation(N, qc_add, ['delay', 0.1])
    th = 2**N // 4
    #threshold of feasible test patterns
    thres_prob_dur = np.partition(pre_sele_dur, -th)[-th]
    thres_prob_delay = np.partition(pre_sele_delay, -th)[-th]
    test_rep_dur = np.zeros(2**num_qubits, dtype = int)
    test_rep_delay = np.zeros(2**num_qubits, dtype = int)
    ## test repetitions obtained by fault simulation
    for index, state in enumerate(states):
        test_rep_dur[index] = math.ceil(hyp_testing(N, qc_add, ['duration',0.1], state))
        test_rep_delay[index] = math.ceil(hyp_testing(N, qc_add, ['delay',0.1], state))
    dur_opt = np.min(test_rep_dur)
    delay_opt = np.min(test_rep_delay)
    ## Check if the minimal repetitions match with pre selection
    for index in range(2**N):
        if(test_rep_dur[index] <= dur_opt * 1.1 and pre_sele_dur[index] >= thres_prob_dur - 1e-3):
            is_match_dur = 1
        if(test_rep_delay[index] <= delay_opt * 1.2 and pre_sele_delay[index] >= thres_prob_delay - 1e-3):
            is_match_delay = 1
    
    return qc_add, is_match_dur, is_match_delay, test_rep_dur, pre_sele_dur, test_rep_delay, pre_sele_delay

dur_match_count = 0
delay_match_count = 0
for i in range(1):
    qc, is_match_dur, is_match_delay, test_rep_dur, pre_sele_dur ,test_rep_delay, pre_sele_delay = protocol_val(6)
    dur_match_count = is_match_dur + dur_match_count
    delay_match_count = is_match_delay + delay_match_count
