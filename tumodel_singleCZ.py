...
params = {"t_tol": 54.2, "t_r": 10.0, "strength_ratio": 0.9,'fault':[None,None]}
circuit.add_gate(Gate("CSIGN", controls = [1], targets=[0], arg_value = params))
device = SCQubits2(num_qubits)
device.load_circuit(circuit)
device.plot_pulses(pulse_labels = [{"sz0":"sz0"}])
# Dressed frame
H0 = device.get_qobjevo(noisy=True)[0](device.get_full_tlist()[-1])
eigenvalues, dressed_frame_U = compute_dressed_frame(H0)
# Unitary evolution
quobjevo, c_ops = device.get_qobjevo(noisy=True)
U_list = qutip.propagator(
...
for j, U in enumerate(U_list):
    ...
    #numeric_gate_qubits = phase_frame_U * dressed_frame_U.dag() * U * dressed_frame_U
...
plt.plot(device.get_full_tlist(), ZZ_phase_result, linewidth = 2)
plt.xlabel("Gate time [ns]")
plt.ylabel("ZZ phase accumulated")
plt.savefig('acc_ZZ_phase_56.2.pdf')
plt.show()

plt.plot(device.get_full_tlist(), fid_result, linewidth = 2.5, color = 'red')
plt.xlabel("Gate time [ns]")
...
#%%
# Two qubit model with direct coupling (CZ gate error case)
num_qubits = 2
noise_type = ['missingZ','delay','amp_sz','duration','amp_coup']
ZZ_phase_missingZ = []
fid_missingZ = []
arg = np.linspace(0, 0, 1)
start_time = time.time()
for i, scale in enumerate(arg):
# CZ gate drive parameters
    circuit = QubitCircuit(num_qubits)
    params = {"t_tol": 55.7, "t_r": 10.0, "strength_ratio": 0.9, 'fault':['missingZ', None]}
    ...
    numeric_gate_qubits = qutip.Qobj(
        get_subspace_array(U, np.array([0, 1, 3, 4])), 
        dims=[[2, 2], [2, 2]]
    )
    ## ZI phase caused by the sz0 drive
    ZI_phase_correction = (
    ...
    ZZ_phase_missingZ.append(ZZ_phase)
    fid_missingZ.append(fid)
print(fid_missingZ)
#%%
arg = np.linspace(-0.2, 0.2, 81)
plt.plot(arg, fid_delay_hann, linewidth = 2.5, color = 'purple', linestyle = '--', label = 'Fidelity with Hann')
plt.plot(arg, fid_delay_sle, linewidth = 2.5, color = 'c', label = 'Fidelity with Slepian')
plt.title('Gate fidelity with alignment fault', fontweight = 'bold')
plt.xlabel("Delay Ratio", fontfamily = 'Times New Roman', fontweight = 'bold')
plt.ylabel("Gate Fidelity", fontfamily = 'Times New Roman', fontweight = 'bold')
font = {'family': 'serif',
        'weight': 'normal',
        'size': 10,}
plt.legend(prop = font, loc = 'best')
plt.savefig('./CZresults/delay_fid.eps', format = 'eps')
plt.show()
np.savetxt('./CZdata/delaysle.txt', fid_delay_sle)
np.savetxt('./CZdata/delayhann.txt', fid_delay_hann)
#%%
arg = np.linspace(-0.2, 0.2, 81)
plt.plot(arg, fid_duration_hann, linewidth = 2.5, color = 'purple', linestyle = '--', label = 'Fidelity with Hann')
plt.plot(arg, fid_duration_sle, linewidth = 2.5, color = 'c', label = 'Fidelity with Slepian')
plt.title('Gate fidelity with duration fault', fontweight = 'bold')
plt.xlabel("Duration Ratio", fontfamily = 'Times New Roman', fontweight = 'bold')
plt.ylabel("Gate Fidelity", fontfamily = 'Times New Roman', fontweight = 'bold')
font = {'family': 'serif',
        'weight': 'normal',
        'size': 10,}
plt.legend(prop = font, loc = 'best')
plt.savefig('./CZresults/duration_fid.eps', format = 'eps')
plt.show()
np.savetxt('./CZdata/durationsle.txt', fid_duration_sle)
np.savetxt('./CZdata/durationhann.txt', fid_duration_hann)
arg1 = np.linspace(-0.1, 0.2, 81)
arg2 = np.linspace(-0.05, 0.2, 81)
plt.plot(arg1, fid_amp_sz_hann, linewidth = 2.5, color = 'purple', linestyle = '--', label = 'Fidelity with Hann')
plt.plot(arg2, fid_amp_sz_sle, linewidth = 2.5, color = 'c', label = 'Fidelity with Slepian')
plt.title('Gate fidelity with sz amplitude fault', fontweight = 'bold')
plt.xlabel("Sz Amplitude Deviation", fontfamily = 'Times New Roman', fontweight = 'bold')
plt.ylabel("Gate Fidelity", fontfamily = 'Times New Roman', fontweight = 'bold')
font = {'family': 'serif',
        'weight': 'normal',
        'size': 10,}
plt.legend(prop = font, loc = 'best')
plt.savefig('./CZresults/amp_sz_fid.eps', format = 'eps')
plt.show()
np.savetxt('./CZdata/ampszsle.txt', fid_amp_sz_sle)
np.savetxt('./CZdata/ampszhann.txt', fid_amp_sz_hann)
arg = np.linspace(-0.2, 0.2, 81)
plt.plot(arg, fid_amp_coup_hann, linewidth = 2.5, color = 'purple', linestyle = '--', label = 'Fidelity with Hann')
plt.plot(arg, fid_amp_coup_sle, linewidth = 2.5, color = 'c', label = 'Fidelity with Slepian')
plt.title('Gate fidelity with coupling strength fault', fontweight = 'bold')
plt.xlabel("Coupling Strength Deviation", fontfamily = 'Times New Roman', fontweight = 'bold')
plt.ylabel("Gate Fidelity", fontfamily = 'Times New Roman', fontweight = 'bold')
font = {'family': 'serif',
        'weight': 'normal',
        'size': 10,}
plt.legend(prop = font, loc = 'best')
plt.savefig('./CZresults/amp_coup_fid.eps', format = 'eps')
plt.show()
np.savetxt('./CZdata/ampcoupsle.txt', fid_amp_coup_sle)
np.savetxt('./CZdata/ampcouphann.txt', fid_amp_coup_hann)

#%%
# %%
#Plot comparisons of different pulse shapes
circuit = QubitCircuit(num_qubits)
params = {"t_tol": 54.2, "t_r": 10.0, "strength_ratio": 0.9, 'fault':[None, None]}
circuit.add_gate(Gate("CSIGN", controls = [0], targets=[1], arg_value = params))
device = SCQubits2(num_qubits)
device.load_circuit(circuit)
tlist_Gauss= device.pulses[0].tlist
coeff_Gauss = device.pulses[0].coeff
plt.figure(figsize = (6,2.5))
plt.plot(tlist_Hann, coeff_Hann, linewidth = 1.5, color = 'c', label = 'Hann_square', linestyle = '--')
plt.plot(tlist_slepian, coeff_slepian, linewidth = 1.5, color = 'green', label = 'Slepian')
plt.plot(tlist_Gauss, coeff_Gauss, linewidth = 1.5, color = 'purple', label = 'Inv_Gaussian')
plt.title('Optimal pulse for selected pulse shapes')
font = {'family': 'serif',
        'weight': 'normal',
        'size': 10,}
plt.xlabel('Gate time(ns)', fontfamily = 'Times New Roman', fontweight = 'bold')
plt.ylabel('Detuning strength', fontfamily = 'Times New Roman', fontweight = 'bold')
...
fid = qutip.average_gate_fidelity(
        qutip.Qobj(numeric_gate_qubits, dims=[[2, 2], [2, 2]]),
        qutip.Qobj(CZ_ideal, dims=[[2, 2], [2, 2]]),
    ) 
print(fid)
